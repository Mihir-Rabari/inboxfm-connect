...
- **Function impl change** — versioning policy: bump `@inboxfm-connect/shared` minor + changelog entry.
...
- No new HTTP endpoints. Function metadata is bundled in `@inboxfm-connect/shared` and read directly by the frontend; no `/v1/formulas/*` route exists.
