...
description: 'Mirror a project across Inboxfm Connect deployments from CI/CD without Git'
icon: 'arrow-right-arrow-left'
---

The `ap project replace` CLI mirrors a project's flows, table schemas, and folders from one Inboxfm Connect deployment to another using direct API calls. Use it from CI/CD to promote work between independent staging and production instances when Git-based [Project Releases](/admin-guide/guides/project-releases) aren't a fit.
...
- Both deployments must share the same major Inboxfm Connect version, and the destination version must be greater than or equal to the source version.
...
The `activepieces` CLI is shipped with each Inboxfm Connect release; pin the version that matches your destination instance to keep request shapes aligned.
...
4. **Destination preflight** (no writes if any of these fail):
    - Inboxfm Connect version is parsed; same major + `dest >= source`.
...
1 connection(s) need authorization on destination before flows can run:
  - Slack Main (@inboxfm-connect/piece-slack) [externalId=slack_main]
```
...
```jsonc
{
  ...
  "connectionsAwaitingAuthorization": [
    { "externalId": "slack_main", "pieceName": "@inboxfm-connect/piece-slack", "displayName": "Slack Main" }
  ],
  "durationMs": 1240
}
```
...
```jsonc
{
  "errors": [
    ...
    { "kind": "CONNECTION_PIECE_MISMATCH", "externalId": "slack_main", "expectedPieceName": "@inboxfm-connect/piece-slack", "foundPieceName": "@inboxfm-connect/piece-discord" }
  ]
}
```
...
{
  "failures": [
    { "pieceName": "@inboxfm-connect/piece-slack", "version": "1.2.3", "pieceType": "OFFICIAL", "message": "ENGINE_OPERATION_FAILURE: ..." },
