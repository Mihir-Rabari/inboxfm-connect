...
description: "Configure SCIM provisioning from Okta to Inboxfm Connect."
---
...
- Generated an API key from `/platform/security/api-keys` route in the Inboxfm Connect app

## Configure SCIM Connection in Okta

<Steps>
  <Step title="Open your Okta app">
    In Okta Admin Console, open your Inboxfm Connect application (created in first [SSO step](/admin-guide/guides/sso#saml-with-okta)).
...
| Inboxfm Connect (SCIM) | Okta Value |
| --- | --- |
| `userName` | `user.email` |
...
<Steps>
  ...
  <Step>
    ...
    <Warning>
      This step assumes that you already have a field in the Okta user profile that you can map to platformRole in your Inboxfm Connect user profile. If you don't have one, you can create a new field in **Directory -> Profile Editor -> User (default)**.
    </Warning>

     - Back to your Inboxfm Connect application page in **Provisioning -> To App -> Attribute Mappings**
...
In the Inboxfm Connect application page, go to **Provisioning -> To App** and enable the actions you want to be applied to Inboxfm Connect when changes occur in Okta.

![Okta role expression example](/resources/screenshots/scim/okta/provision-actions.png)

Now in the **Assignments** tab you can:
- Choose to provision individual users or groups. Note that groups in Okta will be projects in Inboxfm Connect.

- In case you don't have groups and you want to provision your Okta users at once, you can assign the `Everyone` group.

- When editing/creating users in an assigned group (including `Everyone`), they should be updated in Inboxfm Connect.

- To push groups to Inboxfm Connect, go to the **Push groups** tab and click on the push button, find the group and save.
...
<Tip>
  Created users in Inboxfm Connect will receive a welcome email. When clicked, they will be redirected to sign in with `SAML`.
</Tip>


### Deprovision

Users' state switches to `INACTIVE` in Inboxfm Connect only when they are deactivated in Okta. Suspension or deletion in Okta does not reflect in Inboxfm Connect because of Okta's design.
...
<Warning>
  Deleting a group will delete the whole project in Inboxfm Connect with its flows and connections. Users linked to that group won't be affected.
</Warning>
