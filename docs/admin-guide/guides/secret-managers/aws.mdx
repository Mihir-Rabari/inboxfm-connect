...
description: "Connect AWS Secrets Manager to Inboxfm Connect for centralized secret management"
---
...
Create an IAM policy that grants read access to the secrets Inboxfm Connect will retrieve.
...
<Tip>
For production, scope `Resource` to the specific secret ARNs Inboxfm Connect needs instead of using `"*"`.
</Tip>

3. Click **Next**, name the policy (e.g. `InboxfmConnectSecretsReadOnly`), and create it.
...
## Step 3 — Connect in Inboxfm Connect
...
Inboxfm Connect will use the configured credentials to retrieve the secret value and inject it into the connection at runtime.
