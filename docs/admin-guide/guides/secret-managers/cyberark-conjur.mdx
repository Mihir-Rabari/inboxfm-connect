...
description: "Connect CyberArk Conjur to Inboxfm Connect for centralized secret management"
---

CyberArk Conjur is a secrets management solution that provides secure storage and access to credentials. Integration with Inboxfm Connect uses **host/API key authentication**: Inboxfm Connect authenticates as a Conjur host, receives a short-lived token, and uses it to retrieve secrets for which that host has `read` and `execute` permissions.
...
- A Conjur policy that defines a host for Inboxfm Connect and grants it access to the variables you want to use

## Conjur host configuration for Inboxfm Connect

To allow Inboxfm Connect to read secrets, configure a Conjur policy that declares a group, variables, a host, a layer, and the right permissions. The steps below describe how to create that policy file.

### Example policy (Inboxfm Connect)
...
3. **Declare the host** that Inboxfm Connect will use (e.g. `activepieces`) and a **layer** (e.g. `activepieces`), and add the host to the layer:
...
5. **Load the policy** into Conjur. Conjur will create the host and return an **API key** for that host. You will use this API key and the host identity when connecting Inboxfm Connect.
...
Store the **api_key** securely; you will enter it in Inboxfm Connect as the **API Key**.
...
Inboxfm Connect will authenticate as the configured host and retrieve the secret from Conjur when the flow runs.
