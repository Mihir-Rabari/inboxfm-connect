...
To make the action readable by Inboxfm Connect, add it to the array of actions in the piece definition.
...
<Tip>
If the build fails, try debugging by running `npx turbo run build --filter=@inboxfm-connect/piece-gelato`.
It will display any errors in your code.
</Tip>

To test the action, use it from the Inboxfm Connect UI. It should function as shown in the screenshot.
