...
Inboxfm Connect builds every piece into a **self-contained bundle**. Instead of shipping a piece that depends on `@inboxfm-connect/shared`, `@inboxfm-connect/pieces-framework`, `@inboxfm-connect/pieces-common`, and the `@inboxfm-connect/core-*` packages at install time, the build inlines all of that code into a single artifact.
...
- **Inlines all `@inboxfm-connect/*` workspace libraries** (`shared`, `pieces-framework`, `pieces-common`, `core-utils`, `core-piece-types`, …) directly into the bundle. These libraries are **never published to npm** — they only exist as part of each piece's bundle.
...
```jsonc
{
  "name": "@inboxfm-connect/piece-json",
  "version": "0.1.7",
...
No `@inboxfm-connect/*` dependency appears — installing the piece never requires those packages to exist on the registry.
...
```jsonc
{
  "name": "@inboxfm-connect/piece-example",
