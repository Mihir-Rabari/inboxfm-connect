...
<Note>
The Inboxfm Connect monorepo has fully migrated to Turbo. This guide is kept for **historical reference** and for users with older forks that still use the Nx-based build system.
</Note>
...
The Inboxfm Connect monorepo replaced [Nx](https://nx.dev) with [Turbo](https://turbo.build) as its build orchestrator. For pieces, this means:

| | **Old (Nx)** | **New (Turbo)** |
|---|---|---|
| Build config | `project.json` per piece | `package.json` scripts |
| Build command | `nx build pieces-{name}` | `turbo run build --filter=@inboxfm-connect/piece-{name}` |
...
1. **Updates `package.json`** — adds `build` and `lint` scripts, sets `main` and `types` entry points, ensures `@inboxfm-connect/pieces-framework`, `@inboxfm-connect/shared`, and `tslib` are listed as dependencies
...
```bash
npx turbo run build --filter=@inboxfm-connect/piece-your-piece --force
```
