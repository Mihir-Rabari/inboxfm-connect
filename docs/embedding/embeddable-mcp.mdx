...
Every Inboxfm Connect project can act as an [MCP](/mcp/overview) server — a place an AI can connect to in order to run that project's flows.

When you embed Inboxfm Connect, your users log in through **your** app, not Inboxfm Connect. So they can't use the normal "connect your AI" screen (it would ask them to log in to Inboxfm Connect, which they can't).
...
- **Inboxfm Connect** checks everything and gives back a token for that one user's project.

<Info>
It's normal OAuth. The only Inboxfm Connect-specific part is one SDK call (`authorizeMcp`) that shows the Authorize popup inside your app.
</Info>
...
Below, `INSTANCE_URL` is your Inboxfm Connect address (like `https://app.your-company.com`).

## The whole idea in 6 steps

1. **Register your app once** → you get a `client_id`.
2. **Make two random codes** (a "verifier" and a "challenge").
3. **Ask Inboxfm Connect to start a connection** → you get an `authRequestId`.
...
<Steps>
  <Step title="1. Register your app (only once)">
    This tells Inboxfm Connect who your app is. Save the `client_id` — it's the same for all users.
