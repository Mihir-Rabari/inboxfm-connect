...
description: "Guide for creating database migrations in Inboxfm Connect"
icon: "database"
---

Inboxfm Connect uses TypeORM as its database driver in Node.js. We support two database types across different editions of our platform.
...
<Steps>
  <Step title="Setup AP_DB_TYPE">
    Set the `AP_DB_TYPE` environment variable to `POSTGRES` after making sure have latest state by running Inboxfm Connect first.
  </Step>
