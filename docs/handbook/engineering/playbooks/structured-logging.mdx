...
description: "Field-naming conventions for evlog wide events across the Inboxfm Connect server"
icon: "list-tree"
---

All structured logging on the server goes through [evlog](https://www.evlog.dev) — `logger.{info,warn,error,debug}({ fields }, msg)` and `wideEvent.set/error/timed` from `@inboxfm-connect/server-utils`.
