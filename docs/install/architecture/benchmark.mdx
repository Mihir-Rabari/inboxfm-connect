...
<img src="/resources/diagrams/activepieces-architecture.png" alt="Inboxfm Connect production architecture: client, app, Redis job queue, Postgres, S3, and one-flow-per-worker execution tier" />
...
- **Piece bundles**: official tarballs served from the Inboxfm Connect CDN (`AP_USE_CDN_FOR_BUNDLES=true`)
...
```bash
benchmark/run-gke.sh [requests_per_round] [concurrency] [rounds]
```

The results above were measured on the flow-webhook path, which the headless API no longer has. The script now deploys `benchmark/k8s-sandbox.yaml`, provisions a Text Helper connection and API key, and drives `POST /v1/execute` through the app LoadBalancer with the load runner in `benchmark/gate`. It reports cold-boot latency, warm latency and throughput, and app vs worker CPU. `/v1/execute` runs in the app tier, so scale with `APP_REPLICAS`.

<Tip>
This benchmark runs in `SANDBOX_CODE_ONLY` mode. It does **not** represent the performance of Inboxfm Connect Cloud, which uses a different sandboxing mechanism for multi-tenancy. See [Sandboxing](/install/architecture/sandboxing).
</Tip>
