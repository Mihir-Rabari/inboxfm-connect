---
title: "Overview"
description: "How Inboxfm Connect is put together, and why it runs flows the way it does"
icon: "cube"
---

This section explains **why** Inboxfm Connect works the way it does: the components, the execution model, and the trade-offs behind them. If you want to *do* something (size a deployment, set up SSL, scale workers), start with [Configure & Operate](../configure-operate/production-setup). Come here when you want the reasoning under those choices.
...
**Third Party**:
- **Postgres**: The main database for Inboxfm Connect.
...
To scale Inboxfm Connect, add replicas of the workers, the app, or the Postgres database. A small Redis instance is enough: it handles thousands of jobs per second and rarely becomes a bottleneck. For the concrete sizing model, see [Production Setup](../configure-operate/production-setup).
