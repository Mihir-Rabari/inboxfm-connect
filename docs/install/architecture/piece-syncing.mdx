...
```mermaid
flowchart LR
    REG["npm registry<br/>(@inboxfm-connect/piece-*)"]
...
|---|---|---|
| Official | Inboxfm Connect cloud registry | Auto-sync |
...
Every piece declares a `minimumSupportedRelease` (and optional `maximumSupportedRelease`) in its definition: the range of Inboxfm Connect server releases it works on. The catalog filters pieces against the running server's release, so an out-of-range piece is never listed in the builder and never served from the registry.
