...
async function bundlePiece({ piecePath, distPath, repoRoot }: BundlePieceParams): Promise<BundleResult> {
    ...
    const external = directDepsOf(manifest).filter((dep) => !pass.inlined.has(dep) && !dep.startsWith('@inboxfm-connect/') && !BUNDLE_HELPER_DEPS.has(dep))
...
// Only @inboxfm-connect/* workspace code and relative/absolute imports are always bundled in.
    ...
    return {
        ...
        setup(build) {
            build.onResolve({ filter: /.*/ }, (args) => {
                ...
                if (id.startsWith('@inboxfm-connect/')) {
                    return null
                }
...
function workspaceAliases(repoRoot: string): Record<string, string> {
    return {
        ...
        '@inboxfm-connect/core-execution': resolve(repoRoot, 'packages', 'core', 'execution', 'src'),
    }
}
...
// on every engine version while still inlining all @inboxfm-connect/* workspace code.
