...
| `ee/` | Enterprise features | KEEP (except git-repo per PR4D) |
...
### packages/core/execution exports (from index.ts)

```
flow, sample-data, flow-version, test-trigger, properties, operations, note
```

### packages/core/execution/lib/flows/index.ts exports

```
flows/actions/action, flows/operations, flows/operations/paste-operations,
flows/triggers/trigger, flows/triggers/trigger-events/trigger-events-dto,
flows/triggers/trigger-events/trigger-event, flows/triggers/trigger-run,
flows/flow-version, flows/flow, flows/dto/count-flows-request,
flows/dto/create-flow-request, flows/dto/list-flows-request,
flows/dto/flow-mcp.requests, flows/sample-data, flows/folders/folder,
flows/folders/folder-requests, flows/util/flow-structure-util,
flows/util/flow-piece-util, flows/util/flow-canvas-util,
flow-run/dto/list-flow-runs-request, flow-run/execution, flow-run/flow-run,
flow-run/test-flow-run-request, flow-run/log-serializer, flow-run/waitpoint,
engine, engine/rpc, workers/chat-agent-events, workers/job-data,
workers/worker-contract, workers, agents
```

### packages/core/execution/lib/agents/index.ts exports
...
**Issue:** MCP cannot be modified (except dead imports). `PopulatedFlow` is flow-specific (REMOVE per task). Deleting `PopulatedFlow` would break MCP.
...
**Issue:** Template imports `FlowVersion` which is flow-specific (REMOVE per task). Deleting `FlowVersion` would break Template.
...
|------|--------|-------------|--------|
| `FlowVersion` | Template | ❌ NO | Breaks Template module |
| `PopulatedFlow` | MCP | ❌ NO | Breaks MCP module |
...
The following files/directories are NOT referenced by strategic modules:
...
| `waitpoint/` | DELETE candidate - **Waitpoint in REMOVE list** |
...
| File/Directory | Status |
|----------------|--------|
| `chat-agent-events.ts` | ⚠️ AUDIT |
| `job-data.ts` | ⚠️ AUDIT |
| `worker-contract.ts` | ⚠️ AUDIT |
...
```
engine-operation, engine-contract, requests, engine-constants, execution-errors, ExecutionMode enum
```

**Classification:** ✅ KEEP - Runtime engine contracts

---

## Documents Generated

1. `CORE_EXECUTION_AUDIT.md` - This file
2. `CORE_EXECUTION_DEPENDENCIES.md` - (pending dependency graph)
3. `PR4E_CORE_PACKAGE_CLEANUP.md` - (pending final report)

---

**Status:** 🚨 STOPPED - Blocker detected. Cannot safely delete flow-specific types without breaking strategic modules (MCP, Template).