import { FlowTriggerType, isNil, PopulatedFlow } from "@inboxfm-connect/pieces-framework";
import { FlowsContext, ListFlowsContextParams } from "@inboxfm-connect/pieces-framework";
...
export async function listFlowsWithSubflowTrigger({
    ...
    const flows = allFlows.filter(
        (flow) =>
            ...
            '@inboxfm-connect/piece-subflows'
    );
    return flows;
}
