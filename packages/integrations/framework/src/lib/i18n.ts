import { I18nForIntegration, IntegrationMetadataModel, IntegrationMetadataModelSummary } from "./piece-metadata"
import { LocalesEnum } from "@inboxfm-connect/core-utils"
import { MAX_KEY_LENGTH_FOR_CORWDIN } from "@inboxfm-connect/core-piece-types"
import path from 'path';
import fs from 'fs/promises';

export const integrationTranslation = {
  translateIntegration: <T extends IntegrationMetadataModelSummary | IntegrationMetadataModel>(params: TranslateIntegrationParams<T>): T => {
    ...
    try {
      ...
      integrationTranslation.pathsToValuesToTranslate.forEach(key => {
        translateProperty(translatedPiece, key, target)
      })
      return translatedPiece
    }
    catch (err) {
      console.error(`error translating integration ${piece.name}:`, err)
      return piece
    }
  },

  /**Gets the integration metadata regardless of integration location (node_modules or dist), wasn't included inside integration.metadata() for backwards compatibility issues*/
  initializeI18n: async (pieceOutputPath: string): Promise<I18nForIntegration | undefined> => {
    try {
      const locales = Object.values(LocalesEnum);
      const i18n: I18nForIntegration = {};
...
const readLocaleFile = async (locale: LocalesEnum, pieceOutputPath: string) => {
  ...
  try {
    ...
    throw new Error(`Invalid i18n file format for ${locale} in integration ${pieceOutputPath}`);
  } catch (error) {
    console.error(`Error reading i18n file for ${locale} in integration ${pieceOutputPath}:`, error);
    return null;
  }
}

type TranslateIntegrationParams<T extends IntegrationMetadataModelSummary | IntegrationMetadataModel> = {
  piece: T
  locale?: LocalesEnum
  mutate?: boolean
}
...
export const pieceTranslation = {
  ...
  translatePiece: integrationTranslation.translateIntegration,
};
