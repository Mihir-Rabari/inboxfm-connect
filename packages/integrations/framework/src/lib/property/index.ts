...
export { IntegrationAuth,getAuthPropertyForValue } from './authentication';
export type { ExtractIntegrationAuthPropertyTypeForMethods } from './authentication';
...
export { IntegrationAuthProperty, DEFAULT_CONNECTION_DISPLAY_NAME} from './authentication';
...
export const IntegrationProperty = z.union([InputProperty, IntegrationAuthProperty])
export type IntegrationProperty = InputProperty | IntegrationAuthProperty;
export {CustomProperty} from './input/custom-property'
export type {CustomPropertyCodeFunctionParams} from './input/custom-property'
export const IntegrationPropertyMap = z.record(z.string(), IntegrationProperty)
export interface IntegrationPropertyMap {
  [name: string]: IntegrationProperty;
}
...
export { integrationPropertiesUtils, integrationPropertiesUtils as piecePropertiesUtils } from './util';

export type IntegrationPropValueSchema<T extends IntegrationProperty> =
  T extends undefined
  ? undefined
...
export type StaticPropsValue<T extends IntegrationPropertyMap> = {
  [P in keyof T]: IntegrationPropValueSchema<T[P]>;
};
