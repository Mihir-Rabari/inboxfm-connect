import { IntegrationPropertyMap } from ".";
import { IntegrationAuthProperty } from "./authentication";
import { PropertyType } from "./input/property-type";
import * as z from "zod/mini";
import { isEmpty, isNil } from "@inboxfm-connect/core-utils";
import { AUTHENTICATION_PROPERTY_NAME } from "@inboxfm-connect/core-piece-types";

function buildSchema(props: IntegrationPropertyMap, auth: IntegrationAuthProperty | IntegrationAuthProperty[] | undefined, requireAuth: boolean | undefined = true) {
    const entries = Object.entries(props);
    const propsSchema: Record<string, z.ZodMiniType> = {};
    for (const [name, val] of entries) {
      const property = val as any;
  ...
  export const integrationPropertiesUtils = {
    buildSchema
  }
