import { AIProviderName, isNil } from '@inboxfm-connect/core-utils'
import { ApEdition, FlowAction, FlowActionType, flowStructureUtil, FlowVersion, PieceAction } from '@inboxfm-connect/shared'
...
export const migrateV9AiPieces: Migration = {
    ...
    migrate: async (flowVersion: FlowVersion): Promise<FlowVersion> => {
        const newVersion = flowStructureUtil.transferFlow(flowVersion, (step) => {
            ...
            if (step.settings.pieceName === '@inboxfm-connect/piece-text-ai') {
                return migrateTextai(step)
            }
            if (step.settings.pieceName === '@inboxfm-connect/piece-utility-ai') {
                return migrateUtilityAction(step)
            }
            if (step.settings.pieceName === '@inboxfm-connect/piece-image-ai') {
