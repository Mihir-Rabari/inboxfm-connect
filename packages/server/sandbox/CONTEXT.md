...
Bundles are **stored forever** (no eviction/TTL) — small, immutable, write-once, so cheap to keep; this also means zero invalidation logic. State of record is a nullable `bundleFileId` on the `flow_version` row (write-once; no build-version dimension). Upload + registration happen through a **single idempotent API endpoint** that stores the bytes via `fileService` (S3 PUT redirect or DB) and sets `bundleFileId` in the same handler, under the build lock.

**Delete-on-owning-data-deletion (project/platform scope, implemented):** the "optional future" note that used to sit here has been implemented at the project/platform scope — see `.agents/features/file-storage.md#flow-bundle-cleanup-flowbundlecleanupservice` for `flowBundleCleanupService` and its wiring into `hardDeleteProjectHandler` / `hardDeletePlatformHandler`. **Flow-level deletion cleanup is not implemented and is not currently possible**: this fork of the codebase (`Inboxfm Connect`) does not carry over upstream Activepieces' `flow`/`flow_version` entities, flow controllers, worker fleet, or BullMQ job queue into `packages/server/api` — there is no flow-delete endpoint to hook into, and nothing in the live API today ever writes a `FLOW_BUNDLE` file (the API-side `WorkerToApiContract` handlers referenced below, e.g. `worker-rpc-service.ts`, do not exist in this fork; `packages/server/sandbox` and `packages/server/engine` remain buildable/testable in isolation but are not wired into the running API). The project/platform-scoped cleanup is forward-compatible with a future reintroduction of flow-scoped bundles: it is keyed on `FileType.FLOW_BUNDLE` + `projectId`/`platformId`, not on any flow concept, so it needs no changes if flow-level bundle creation returns.
