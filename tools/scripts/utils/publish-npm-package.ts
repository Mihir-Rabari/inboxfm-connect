...
// @inboxfm-connect/* library (shared, framework, common, core-*) is inlined and NONE of them is
// published to npm, and there must be no unresolved workspace:* dep. So refuse to publish if any
// dependency is either still a workspace:* range OR an @inboxfm-connect/* package — regardless of how
...
function assertNoUnpublishableDeps(packageJsonPath: string): void {
  ...
  for (const field of depFields) {
    ...
    for (const [name, version] of Object.entries(deps)) {
      ...
      } else if (name.startsWith('@inboxfm-connect/')) {
...
export const publishNpmPackage = async (path: string): Promise<void> => {
  ...
  // @inboxfm-connect/* + workspace deps that are now inlined). MUST be awaited — it copies the
